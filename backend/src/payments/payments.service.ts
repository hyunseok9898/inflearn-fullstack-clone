  ...
  async verifyPayment(verifyPaymentDto: VerifyPaymentDto, userId: string) {
    try {
      ...
      const customData = payment.customData
        ? (JSON.parse(payment.customData) as PaymentCustomData)
        : null;
      ...
      return await this.processPayment(payment, customData, courses, userId);
    } catch (error) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.payment.create({
        data: {
          ...
          status: 'PAID',
          paidAt: new Date(),
          portoneData: payment as Prisma.InputJsonValue,
        },
      });
